...
class RubyACL
  ...
  def create_acl_in_db(connector)
    if(acl_exist(connector))
      
    else
      
      create_acl(connector)
    end
  end
  
  def acl_exist(connector)
    col = connector.getcollection("/db/acl")
    if(col.name == "/db/acl")
      puts "kolekce pro acl existuje"
      
      return true
    else
      puts "kolekce neexistuje."
      return false
    end
  end
